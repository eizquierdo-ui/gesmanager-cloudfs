...
const TipoCambioPage = () => {
  ...
  const handleFormSubmit = async (values) => {
    try {
      if (currentItem) {
        await updateTipoCambio(currentItem.id, values);
      } else {
        // **INICIO DE LA CORRECCIÓN**
        // Creamos un nuevo objeto con los valores del formulario
        const dataToSave = { ...values };
        // Añadimos el empresa_id de la sesión activa
        dataToSave.empresa_id = sessionData.empresa_id;
        // Llamamos a la función de creación con el objeto enriquecido
        await createTipoCambio(dataToSave);
